...
function once (evt, cb) {
  var e = this.eventEmitter.addEventListener(evt, function (data) {
    this.eventEmitter.removeEventListener(evt, e);
    cb.call(this, data);
  }.bind(this));
};
function emit (evt, a) {
  ...
  var data = arguments.length > 1 ? Array.prototype.slice.call(arguments, 1) : [];
  var e = new CustomEvent(evt, { detail: { message: a, data: data }});
...
var net = {
  createConnection: function (port, server) {
    return {
      ...
      eventEmitter: document.createDocumentFragment(),
...
Client.prototype.addListener = function (evt, cb) {
  ...
  this.eventEmitter.addEventListener(evt, function (data) {
    ...
    cb(data.detail.message, data.detail.data);
  });
};
